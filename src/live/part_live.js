...
CG.loadManagerData = async function(){
  ...
  try {
    var q = await Promise.all([
      ...
      CG.sb.from("season_registrations").select("id,profile_id,position,scout_ovr, profiles(gamertag,ea_id)")
    ]);
    if ((q[0]&&!q[0].error) || (q[1]&&!q[1].error)){
      CG.mapDraftData(CG.lg, (q[0]&&!q[0].error&&q[0].data)||[], (q[1]&&!q[1].error&&q[1].data)||[]);
    }
  } catch(e){}
};
