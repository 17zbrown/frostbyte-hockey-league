...
CG.buildLiveLeague = async function(opts){
  ...
  lg._rosterRaw = roster;   /* v2.34: the rights-held checks need every seat's club and origin */
...
CG.roadToFive = function(lg, clubCode){
  ...
  /* "reachable" assumes a player may be dressed in EVERY remaining pre-season game. That is true
     because Rule 5.2's weekly appearance cap does not apply in the pre-season (v2.28). If a cap is
     ever reintroduced there, this arithmetic must subtract it or it will call a player reachable
     who is already mathematically out. */
...
CG.loadManagerData = async function(){
  ...
  try {
    ...
    if (myTid){
      /* v2.38: the Owner's management permissions for this club + the approval queue. Both are
         club-scoped by RLS; a page that cannot read them simply runs at the default (full access,
    ...
    if (!CG._notifChannel){
      CG._notifChannel = CG.sb.channel("notifs-"+CG.auth.user.id)
        .on("postgres_changes",{ event:"INSERT", schema:"public", table:"notifications",
          ...
          /* a seat changed hands (appointed, removed): rebuild so Team HQ appears or goes on its
             own, with the role re-derived — no refresh, nothing the member is doing interrupted */
          if ((n.type === "role" || n.type === "request") && CG.liveReload) CG.liveReload();
        }).subscribe();
    }
...
/* ---------------------------------------------------------------- *
 * Rule 2.2 — contract offers, from the player's side.
...
CG.clubOffersCardHtml = function(){
  ...
  return '<div class="card" style="margin-bottom:18px'+(waiting?';border-color:var(--chrome)':'')+'">'+
    ...
    offs.map(function(o){
      ...
      var ext = !o.immediate && CG.contractOf(o.player_id) && CG.contractOf(o.player_id).team_id===o.from_team_id;
      return '<div class="card-b" style="border-top:1px solid var(--line-soft)">'+
        '<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:8px">'+
          ...
          '<span class="chip'+(theirs?' chip-live':'')+'">'+(theirs?'His number — your move':'Waiting on him')+'</span>'+
          (ext?'<span class="chip chip-chrome">Extension · from S'+esc(String(o.start_season))+'</span>':'')+
        '</div>'+
    ...
    '<div class="card-b" style="border-top:1px solid var(--line)"><span class="caption">Accepting a free agent’s counter signs him immediately at his numbers; accepting an extension ask signs his next deal for the rollover (Rule 2.2). Revising sends the negotiation back to him.</span></div></div>';
};
CG.offersCardHtml = function(){
  ...
  return '<div class="card" style="margin-bottom:18px;border-color:var(--chrome)">'+
    '<div class="card-h"><h3>Contract offer'+(offs.length>1?'s':'')+' for you</h3>'+
    '<span class="chip chip-warn">'+offs.length+' waiting</span></div>'+
    offs.map(function(o){
      ...
      return '<div class="card-b" style="border-top:1px solid var(--line-soft)">'+
        '<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:8px">'+
          ...
          '<span class="chip'+(mine?"":" chip-live")+'">'+(mine?"Your counter is with the club":"Awaiting your answer")+'</span>'+
          (ext?'<span class="chip chip-chrome">Extension</span>':'')+
        '</div>'+
        '<div style="display:flex;gap:18px;flex-wrap:wrap;margin-bottom:6px">'+
          '<span><b class="num" style="font-size:19px">'+CG.fmtMoney(o.salary)+'</b><span class="caption" style="display:block">per season</span></span>'+
          '<span><b class="num" style="font-size:19px">'+o.years+'</b><span class="caption" style="display:block">season'+(o.years>1?'s':'')+'</span></span>'+
          (o.immediate?'<span><b class="num" style="font-size:19px">Now</b><span class="caption" style="display:block">joins the roster</span></span>'
                      :'<span><b class="num" style="font-size:19px">S'+o.start_season+'</b><span class="caption" style="display:block">starts</span></span>')+
        '</div>'+
        (o.note?'<p class="small" style="color:var(--steel);font-style:italic">\u201c'+esc(o.note)+'\u201d</p>':"")+
        (mine
          ? '<p class="caption" style="margin-top:8px">You asked for '+CG.fmtMoney(o.salary)+' \u00d7 '+o.years+'. The club decides next.</p>'
          : '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;justify-content:flex-end">'+
              '<button class="btn btn-ghost btn-sm" data-offer-deny="'+o.id+'">Decline</button>'+
              '<button class="btn btn-ghost btn-sm" data-offer-counter="'+o.id+'" data-sal="'+o.salary+'" data-yrs="'+o.years+'">Counter</button>'+
              '<button class="btn btn-chrome btn-sm" data-offer-accept="'+o.id+'" data-club="'+esc(nm)+'" data-ext="'+(ext?'1':'')+'">Accept'+(o.immediate?' and sign':(ext?' and re-sign':''))+'</button>'+
            '</div>')+
        '</div>';
    }).join("")+
    '<div class="card-b" style="border-top:1px solid var(--line)"><span class="caption">Accepting a free-agent offer puts you on the club\u2019s roster immediately \u2014 no league approval (Rule 2.2). Accepting an extension signs your next deal and changes nothing this season. Accepting one offer withdraws the rest.</span></div></div>';
};
CG.wireClubOfferActions = function(){
  CG.wireExtensionCard();
  document.querySelectorAll("[data-coffer-accept]").forEach(function(b){ b.addEventListener("click", function(){
    ...
    CG.confirm("Accept "+nm+"’s terms?", isExt?"He re-signs at the numbers he asked for. A next-season deal changes nothing this season and comes into force with next season’s cap year; a rights-held re-sign takes effect the moment it is accepted.":"He signs at the numbers he asked for and joins your roster immediately.", isExt?"Re-sign him":"Accept and sign", function(){
      ...
      CG.sb.rpc("respond_offer",{ p_offer:id, p_action:"accept", p_salary:null, p_years:null }).then(function(r){
        ...
        CG.toast(isExt ? nm+" re-signed — his new deal starts with next season’s cap year" : nm+" signed — welcome aboard","ok");
  ...
  document.querySelectorAll("[data-coffer-counter]").forEach(function(b){ b.addEventListener("click", function(){
    var id=this.getAttribute("data-coffer-counter"), nm=this.getAttribute("data-name"), isExtC=this.getAttribute("data-ext")==="1";
...
CG.wireOfferActions = function(){
  ...
  document.querySelectorAll("[data-offer-accept]").forEach(function(b){ b.addEventListener("click", function(){
    var id=this.getAttribute("data-offer-accept"), club=this.getAttribute("data-club"), btn=this, isExt=this.getAttribute("data-ext")==="1";
    CG.confirm(isExt?"Re-sign with "+club+"?":"Sign with "+club+"?",
      isExt?"This signs your deal with "+club+". A next-season deal changes nothing this season and comes into force with next season’s cap year; a rights-held re-sign takes effect now.":"Accepting puts you on their roster right away and withdraws every other offer you\u2019re holding.",
      isExt?"Re-sign":"Accept and sign", function(){
      btn.disabled=true;
      CG.sb.rpc("respond_offer",{ p_offer:id, p_action:"accept", p_salary:null, p_years:null }).then(function(r){
        btn.disabled=false;
        if(r.error){ CG.toast(r.error.message,"err"); return; }
        CG.toast(isExt?"Re-signed with "+club+" \u2014 see you next season":"Signed with "+club+" \u2014 welcome aboard","ok");
        CG.loadMyOffers().then(function(){ CG.reloadLeague(); });
      });
    });
  }); });
  document.querySelectorAll("[data-offer-deny]").forEach(function(b){ b.addEventListener("click", function(){
    var id=this.getAttribute("data-offer-deny"), btn=this;
    CG.confirm("Decline this offer?","The club is told you passed. They can always send another.","Decline", function(){
      btn.disabled=true;
      CG.sb.rpc("respond_offer",{ p_offer:id, p_action:"deny", p_salary:null, p_years:null }).then(function(r){
        btn.disabled=false;
        if(r.error){ CG.toast(r.error.message,"err"); return; }
  ...
  document.querySelectorAll("[data-offer-counter]").forEach(function(b){ b.addEventListener("click", function(){
...
CG._wrapHubDashboard = function(){
  ...
  CG.hubDashboard = function(){
    ...
    var mt = CG.myManagedTeam && CG.myManagedTeam();
    if (mt) return offers + CG._mgmtDashboard(mt);
    ...
    var h = '<div style="margin-bottom:24px"><span class="eyebrow chr">'+CG.fmtFull(CG.now())+'</span>'+
      ...
      '<p class="lede" style="margin-top:10px">You’re signed in — here’s where you stand on the way to a roster spot.</p></div>'+offers;
...
CG.ROUTES["pickup-import"] = function(){
  var head = CG.pageHead("Pickup stats", "Import a pickup game",
    "Played a pickup between two EASHL clubs? Search your club, pick the game from EA’s recent match list, and its box score attaches to each player’s profile as pickup stats — kept separate from league play (it never affects eligibility, standings, or overall).");
  if (!CG.auth || !CG.auth.profile){
...
CG.ROUTES.register = function(){
  ...
  /* active first; else a deal SIGNED for this season in last season's exclusive window (v2.34) —
     the same status set contractHeldIds and the database's seating paths use */
  var myCt = ((CG.lg && CG.lg._contractsRaw) || []).find(function(c){
    return p && c.profile_id===p.id && c.status==="active" && !c.is_manager && c.team_id &&
           (c.start_season||1)<=snumR && (c.end_season||1)>=snumR; })
      || ((CG.lg && CG.lg._contractsRaw) || []).find(function(c){
    return p && c.profile_id===p.id && c.status==="signed" && !c.is_manager && c.team_id &&
  ...
  var statusCard = reg ? '<div class="note grn" style="margin-bottom:18px"><b style="font-family:var(--f-disp)">You’re registered for Season '+(s.number||1)+'.</b> '+(myCt&&onRoster
      ...
      '</div>' : "";
  if (myCt && !reg){
    statusCard = '<div class="note" style="margin-bottom:18px"><b style="font-family:var(--f-disp)">'+(myCt.status==="signed"
        ? 'Your next deal with '+esc(ctName)+' is signed through Season '+(myCt.end_season||snumR)+' — it comes into force with Season '+(myCt.start_season||snumR)+'’s cap year, and a contract doesn’t replace registration.'
        : 'You’re under contract with '+esc(ctName)+' through Season '+(myCt.end_season||snumR)+' — but a contract doesn’t replace registration.')+'</b> Until you sign up you can’t play, and your '+CG.fmtMoney(myCt.salary||0)+' salary sits on the club’s cap as dead money. If '+esc(ctName)+' takes on a new owner and you still haven’t signed up after the deadline, the deal is voided and you’re suspended through Season '+(myCt.end_season||snumR)+' (Rule 2.5). Registering — any time — puts you straight back on the roster.</div>' + statusCard;
  }
...
CG.draftPickModalLive = function(pickId, forCode){
  ...
  function wire(){
    document.querySelectorAll("[data-modpick]").forEach(function(b){ b.addEventListener("click", function(){
      ...
      CG.mgmtQueue("draft_make_pick", { p_pick:pickId, p_player:pid }, "draft "+nm+" with the pick on the clock").then(function(q){ if (q){ btn.disabled=false; btn.textContent="Draft"; return; }
      CG.sb.rpc("draft_make_pick",{ p_pick:pickId, p_player:pid }).then(function(r){
...
CG.AFTER._hubDraft = function(){
  ...
  document.querySelectorAll("[data-quickpick]").forEach(function(b){ b.addEventListener("click", function(){
    ...
    btn.disabled=true;
    CG.mgmtQueue("draft_make_pick", { p_pick:pickId, p_player:pid }, "draft "+nm+" with the pick on the clock").then(function(q){ if (q){ btn.disabled=false; return; }
    CG.sb.rpc("draft_make_pick",{ p_pick:pickId, p_player:pid }).then(function(r){
...
CG.admDraftLive = function(){
  ...
  h += '<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:18px">'+
    ...
    '<div class="kpi" style="cursor:default"><b class="num">'+pool.length+'</b><span>in the draft pool</span></div></div>';
...
CG.hubShell = function(section, inner){
  /* v2.38: the Owner-approves banner sits above the page it governs, for every role */
  var ban = CG.mgmtApprovalBanner ? CG.mgmtApprovalBanner(section) : "";
  if (ban) inner = ban + inner;
...
/* ================================================================
   ...
   ================================================================ */
/* The role is derived from the seat pointers (CG.computeRole), but it was computed ONCE, on the
   auth event. A GM approved while signed in, or one the Owner removed, kept the old role until a
   hard refresh: the Team HQ tab (which reads the seats live) and the routes behind it (which read
   CG.auth.role) disagreed. Every league rebuild now re-derives it. A "View as…" preview is left
   alone — it is deliberately not reality. */
CG.refreshRole = function(){
  if (CG._va || !CG.auth || !CG.auth.user || !CG.auth.profile || !CG.computeRole) return false;
  var r = CG.computeRole(CG.auth.profile);
  if (r === CG.auth.role) return false;
  CG.auth.role = r;
  return true;
};
/* ================================================================
   v2.38 · MANAGEMENT PERMISSIONS (Rule 2.6)
   The Owner sets, per Team HQ page and per seat (GM, AGM), one of three modes:
     full     — the seat acts freely (the default; nothing changes for a club that never sets this)
     approve  — the seat sees the page, but every move on it is queued for the Owner's approval
     hidden   — the page is withheld from the seat, and its moves are refused
   The database is the authority (public.mgmt_access_for / mgmt_gate on every management write);
   this mirror only decides what to draw and where to send a move.
   ================================================================ */
CG.MGMT_PAGES = [
  ["roster",     "Roster",         "Waivers, extension offers, trade-block flags, camp moves", true],
  ["lines",      "Lineup builder", "Saved lines, night plans and per-game lineups", true],
  ["schedule",   "Schedule",       "Server vetoes and preferred picks", true],
  ["gamestats",  "Game stats",     "Box-score repairs — reviewed by the statistics staff, so there is no approval mode", false],
...
if (!CG.fmtAgo) CG.fmtAgo = function(ts){
  var d = Math.max(0, (CG.now ? CG.now() : Date.now()) - ts), m = Math.round(d/60000);
...
/* mirror of public.mgmt_access_for — what the signed-in member may do on one Team HQ page */
CG.mgmtAccess = function(page){
  var seat = CG.mySeat();
...
CG.mgmtPendingCount = function(){
  var seat = CG.mySeat(); if (!seat) return 0;
  var uid = CG.auth.user && CG.auth.user.id;
  return CG.mgmtMoves("pending").filter(function(m){ return seat==="owner" || m.requested_by===uid; }).length;
};
/* Every management write passes here first. Under "Owner approves" the move is queued for the
...
CG.mgmtApprovalBanner = function(section){
  var page = section==="lineup" ? "lines" : section;
  if (!CG.MGMT_ACTION_PAGE || !CG.MGMT_PAGES.some(function(x){ return x[0]===page; })) return "";
...
CG.withdrawMgmtMove = function(id){
  CG.sb.rpc("mgmt_withdraw_move", { p_id:id }).then(function(r){
    if (r.error){ CG.toast("Couldn’t withdraw: "+r.error.message,"err"); return; }
    CG.toast("Move withdrawn","ok"); CG.reloadLeague();
  });
};
...
CG.mgmtSeatsTable = function(m){
  ...
  function row(role,label,required){
    ...
    /* Rule 2.6 (v2.38): the Owner alone nominates and removes the GM and AGM. A held seat offers
       the Owner "Remove"; a GM or AGM reading the same table sees who decides it. */
    var action=owner?'<span class="caption">—</span>'
      :pend?'<span class="caption">'+esc((pend.nominee&&pend.nominee.gamertag)||"nominee")+'</span>'
      :holder?(m.isOwner?'<button class="btn btn-ghost btn-sm" data-remove-role="'+role+'" data-remove-name="'+esc(holder)+'">Remove</button>'
                        :'<span class="caption">Only the Owner changes this seat</span>')
      :m.isOwner?'<button class="btn '+(required?"btn-chrome":"btn-ghost")+' btn-sm" data-nominate-role="'+role+'">Nominate</button>'
      :'<span class="caption">Only the Owner nominates</span>';
    return '<tr>'+
...
CG.mgmtMoveRow = function(mv, forOwner){
  var names=(CG.lg&&CG.lg._profName)||{};
  var who=(mv.requester&&mv.requester.gamertag)||names[mv.requested_by]||"Manager";
  var seat=(function(){ var t=CG.myManagedTeam(); return !t?"":t.gm===mv.requested_by?"GM":t.agm===mv.requested_by?"AGM":""; })();
  var st={pending:'<span class="chip chip-warn chip-xs">Waiting</span>',approved:'<span class="chip chip-win chip-xs">Approved</span>',denied:'<span class="chip chip-xs">Denied</span>',
          withdrawn:'<span class="chip chip-xs">Withdrawn</span>',failed:'<span class="chip chip-xs" style="color:var(--red-ink);border-color:var(--red-ink)">Could not be made</span>'}[mv.status]||esc(mv.status);
  var when=mv.status==="pending"?CG.fmtAgo(Date.parse(mv.created_at)):(mv.decided_at?CG.fmtAgo(Date.parse(mv.decided_at)):"");
  var tail=mv.status==="failed"&&mv.result?'<div class="caption" style="color:var(--red-ink);margin-top:2px">'+esc(mv.result)+'</div>':(mv.note?'<div class="caption" style="margin-top:2px">Note: '+esc(mv.note)+'</div>':'');
  var act=mv.status!=="pending"?'':forOwner
    ?'<span style="display:flex;gap:6px"><button class="btn btn-chrome btn-sm" data-mgmt-decide="'+esc(mv.id)+'" data-approve="1">Approve</button><button class="btn btn-ghost btn-sm" data-mgmt-decide="'+esc(mv.id)+'" data-approve="0">Deny</button></span>'
    :(mv.requested_by===(CG.auth.user||{}).id?'<button class="btn btn-ghost btn-sm" data-mgmt-withdraw-move="'+esc(mv.id)+'">Withdraw</button>':'');
  return '<div class="titem" style="align-items:flex-start"><span style="flex:1;min-width:0"><b style="font-family:var(--f-disp)">'+esc(mv.summary)+'</b>'+
    '<div class="caption">'+esc(who)+(seat?' · '+seat:'')+' · '+esc(CG.mgmtPageLabel(mv.page))+' · '+esc(when)+'</div>'+tail+'</span>'+st+act+'</div>';
};
CG.mgmtApprovalsCard = function(m){
  var uid=(CG.auth.user||{}).id, all=CG.mgmtMoves();
  var pending=all.filter(function(x){ return x.status==="pending" && (m.isOwner || x.requested_by===uid); });
  var recent=all.filter(function(x){ return x.status!=="pending" && (m.isOwner || x.requested_by===uid); }).slice(0,8);
  if (!m.isOwner && !pending.length && !recent.length) return "";
  return '<div class="card" style="margin-bottom:18px"><div class="card-h"><h3>'+(m.isOwner?"Approvals":"Your moves awaiting the Owner")+'</h3>'+
    (pending.length?'<span class="chip chip-warn chip-xs">'+pending.length+' waiting</span>':'<span class="chip chip-xs">Nothing waiting</span>')+'</div>'+
    '<div class="tasklist">'+(pending.length?pending.map(function(x){ return CG.mgmtMoveRow(x, m.isOwner); }).join("")
      :'<div class="titem"><span class="caption">'+(m.isOwner?"Moves your GM or AGM make on a page set to “Owner approves” land here. Approving runs the move at once; denying tells them why.":"Nothing of yours is waiting.")+'</span></div>')+
    '</div>'+(recent.length?'<div class="card-b" style="border-top:1px solid var(--line)"><span class="rb-lab" style="display:block;margin-bottom:6px">Recent decisions</span><div class="tasklist">'+recent.map(function(x){ return CG.mgmtMoveRow(x, false); }).join("")+'</div></div>':'')+'</div>';
};
CG.mgmtPermissionsCard = function(m){
  ...
  var rows=CG.MGMT_PAGES.map(function(pg){
    ...
    return '<tr><td class="tleft"><b style="font-family:var(--f-disp)">'+esc(pg[1])+'</b><div class="caption">'+esc(pg[2])+'</div></td>'+cells+'</tr>';
  }).join("");
...
CG.AFTER._management = function(){
  /* v2.38: the Owner's approval queue */
  document.querySelectorAll("[data-mgmt-decide]").forEach(function(b){ b.addEventListener("click", function(){
    var id=this.getAttribute("data-mgmt-decide"), ok=this.getAttribute("data-approve")==="1";
    var mv=CG.mgmtMoves().find(function(x){ return x.id===id; })||{summary:"this move"};
    CG.modal((ok?"Approve — ":"Deny — ")+esc(mv.summary),
      '<p class="caption" style="margin-bottom:12px">'+(ok?"The move is made the moment you approve it, through the league’s usual checks. If it can no longer be made, you both see why.":"Nothing changes; your manager is told.")+'</p>'+
      '<label class="fld"><span>Note to your manager (optional)</span><input id="mgmtDecideNote" maxlength="200" placeholder="'+(ok?"Go ahead":"Not this week")+'"></label>',
      '<button class="btn btn-ghost" data-close>Cancel</button><button class="btn '+(ok?"btn-chrome":"btn-ink")+'" id="mgmtDecideGo">'+(ok?"Approve and make it":"Deny")+'</button>');
    document.getElementById("mgmtDecideGo").addEventListener("click", function(){
      var note=((document.getElementById("mgmtDecideNote")||{}).value||"").trim()||null, btn=this; btn.disabled=true;
      CG.sb.rpc("mgmt_decide_move",{ p_id:id, p_approve:ok, p_note:note }).then(function(r){
        btn.disabled=false;
        if(r.error){ CG.toast("Couldn’t decide: "+r.error.message,"err"); return; }
        if(CG.closeOverlay) CG.closeOverlay();
        var d=r.data||{};
        if (d.status==="failed") CG.toast("Approved, but it could not be made: "+(d.error||"the league’s checks refused it"),"err");
        else CG.toast(ok?"Approved — the move is made":"Denied — your manager has been told","ok");
        CG.reloadLeague();
      });
    });
  }); });
  /* v2.38: the permissions matrix — local edits, one save, honest status */
  var card=document.getElementById("permCard");
  if (card && card.querySelector("[data-perm-seat]")){
    var draft={}, base={};
    card.querySelectorAll(".seg-b.on").forEach(function(b){ (base[b.getAttribute("data-perm-seat")]=base[b.getAttribute("data-perm-seat")]||{})[b.getAttribute("data-perm-page")]=b.getAttribute("data-perm-mode"); });
    draft=JSON.parse(JSON.stringify(base));
    var saveB=document.getElementById("permSave"), resetB=document.getElementById("permReset"), stat=document.getElementById("permStatus");
    function dirty(){ return JSON.stringify(draft)!==JSON.stringify(base); }
    function paint(){
    ...
    if (saveB) saveB.addEventListener("click", function(){
      var m=CG.clubMgmt(); if(!m||!m.isOwner){ CG.toast("Only the club’s Owner sets management permissions","err"); return; }
  ...
  document.querySelectorAll("[data-remove-role]").forEach(function(b){ b.addEventListener("click", function(){
    CG.removeManager(this.getAttribute("data-remove-role"), this.getAttribute("data-remove-name")||"this manager");
  }); });
...
/* Rule 2.6 (v2.38): the Owner removes a sitting GM or AGM. Owner-only in the database too
   (owner_remove_manager checks teams.owner_profile_id = auth.uid()), so this button grants nothing
   the seat does not already carry. Fail-loud: a refused call is a toast, never a silent no-op. */
CG.removeManager = function(role, name){
  var m = CG.clubMgmt(); if(!m || !m.isOwner){ CG.toast("Only the club’s Owner can remove management","err"); return; }
  ...
  CG.confirm("Remove "+name+" as "+label+"?",
    ...
    "Remove", function(){
      CG.sb.rpc("owner_remove_manager", { p_team_code:m.club, p_role:role }).then(function(r){
        if(r.error){ CG.toast("Couldn’t remove: "+r.error.message,"err"); return; }
        CG.toast(name+" removed as "+label,"ok"); CG.reloadLeague();
      });
    });
};
...
CG.seatConflict = function(app, lg){
  ...
  if (holder && holder!==nominee) return { kind:"filled", chip:seat+" seat filled",
    text:"The "+label+" seat at "+club+" is held by "+who(holder)+". A club holds "+art+" "+label+
         " (Rule 2.6), so approving this will not seat "+nomName+" — the club’s Owner removes the sitting manager (or a commissioner vacates the seat) first." };
...
CG.contractHeldIds = function(){
  ...
  ((CG.lg && CG.lg._contractsRaw) || []).forEach(function(c){
    ...
    /* v2.34: a deal SIGNED for this season during last season's exclusive window holds the player
       just as firmly — he is never a draft prospect or a free agent for a season he has signed for */
    if (c.status==="signed" && c.team_id && (c.start_season||1)<=sn && (c.end_season||1)>=sn) out[c.profile_id]=true;
  });
  return out;
};

/* ---- Contract extensions (Rule 2.2 / 2.5, v2.34) ----
   The commissioner's rulings, verbatim: "Teams can resign/extend player contracts any time within
   the last full season of their contracts, just like how the NHL handles it." and "Only during the
   free agency period is when players can talk with other outside teams." The cap year runs from
   one free-agency opening to the next, so a deal is in its final season from the moment that
   season's free agency opens, and may be extended at any point in it. The database enforces both
   halves: offer_extension / request_extension refuse before free agency has opened, and
   offer_free_agent refuses anyone under contract, so an outside club cannot reach a player until
   his deal has ended. These mirror the window so the site shows the right state, never a button
   the server would refuse. */
CG.capYearOpensAt = function(){
...
CG.contractOf = function(pid){
  ...
  return ((CG.lg && CG.lg._contractsRaw) || []).find(function(c){
    return c.profile_id===pid && c.status==="active" && !c.is_manager && c.team_id && (c.start_season||1)<=sn && (c.end_season||1)>=sn; }) || null;
};
/* Rights held: between the rollover and this season's free agency, a club may still re-sign the
   player whose deal just ended with last season — mirrors _extendable_contract()'s second branch. */
CG.rightsHeldContractOf = function(pid){
  ...
  return ((CG.lg && CG.lg._contractsRaw) || []).find(function(c){
    return c.profile_id===pid && c.status==="expired" && !c.is_manager && c.team_id && (c.end_season||1)===sn-1; }) || null;
};
/* a seat this season on another club that is NOT the pre-season's temporary loan */
CG.seatedElsewhere = function(pid, teamId){
  var sid = CG.SEASON && CG.SEASON.id;
  return ((CG.lg && CG.lg._rosterRaw) || []).some(function(rs){
    return rs.profile_id===pid && (!sid || rs.season_id===sid) && rs.team_id!==teamId && (rs.origin||"")!=="preseason_random"; });
};
...
CG.extendableContractOf = function(pid){
  ...
  if (r && !CG.signedExtensionOf(pid) && !c && !CG.seatedElsewhere(pid, r.team_id)) return r;
  if (r && c && !CG.signedExtensionOf(pid) && !CG.seatedElsewhere(pid, r.team_id)){
    /* his only deal is the pre-season loan's — looked through */
    var loan = ((CG.lg && CG.lg._rosterRaw) || []).some(function(rs){ return rs.profile_id===pid && rs.team_id===c.team_id && (rs.origin||"")==="preseason_random"; });
    if (loan) return r;
  }
  return null;
};
CG.signedExtensionOf = function(pid){
  return ((CG.lg && CG.lg._contractsRaw) || []).find(function(c){ return c.profile_id===pid && c.status==="signed"; }) || null;
};
...
/* the club that holds a player's current deal, as a code */
CG.contractClubCode = function(pid){
  var c = CG.contractOf(pid); if (!c) return null;
  return ((CG.lg && CG.lg._idToCode) || {})[c.team_id] || null;
};

/* The player's own card on his dashboard: where his deal stands, and the one thing he can do
   about it. Rendered only for a rostered player under a non-manager contract. */
CG.extensionCardHtml = function(){
  var uid = CG.auth && CG.auth.user && CG.auth.user.id; if (!uid) return "";
  var c = CG.contractOf(uid);
  if (!c){
    /* post-rollover, pre-free-agency: his deal just ended but his club still holds his rights */
    var rh = CG.rightsHeldContractOf(uid), live0 = (CG._myOffers||[]).filter(function(o){ return !o.immediate; })[0];
    if (!rh) return "";
    var rcode = ((CG.lg && CG.lg._idToCode) || {})[rh.team_id], rclub = (rcode && CG.TEAM[rcode] && CG.TEAM[rcode].name) || "your club";
    var fa = CG.capYearOpensAt();
    var registration = CG.auth && CG.auth.registration;   /* the register link below shows only while !registration */
    return '<div class="card" style="margin-bottom:18px"><div class="card-h"><h3>Your contract</h3><span class="chip chip-warn">Rights held by '+esc(rcode||rclub)+'</span></div><div class="card-b">'+
      '<p class="small" style="color:var(--steel)">Your '+CG.fmtMoney(rh.salary)+' deal with <b>'+esc(rclub)+'</b> ended with last season. Until free agency opens'+(fa?' on <b>'+esc(CG.fmtFull(fa))+'</b>':'')+' only '+esc(rclub)+' can offer you a new one, and you can ask them — no other club may approach you (Rule 2.2). A deal signed now is for <b>this</b> season: it takes effect the moment you accept and is checked against '+esc(rclub)+'’s space this season'+(!registration ? '. <a href="#/register" style="font-weight:700;border-bottom:2px solid var(--chrome)">Register for the season</a> so a signed deal can seat you.' : ', and you are seated on their roster at once.')+' After free agency opens you are a free agent and any club may approach you.</p>'+
      (live0 ? '<p class="small" style="margin-top:8px">'+(CG.offerAwaitsClub(live0)?'Your ask of <b>'+CG.fmtMoney(live0.salary)+' × '+live0.years+'</b> is with '+esc(rclub)+'.':esc(rclub)+' has offered you <b>'+CG.fmtMoney(live0.salary)+' × '+live0.years+'</b> — answer it in the offer card above.')+'</p>'
             : '<button class="btn btn-chrome btn-sm" id="askResign" style="margin-top:12px">'+CG.ic("plus",14)+'Ask to re-sign</button>')+'</div></div>';
  }
  var sn = (CG.SEASON && CG.SEASON.number) || 1, code = CG.contractClubCode(uid);
  var club = (code && CG.TEAM[code] && CG.TEAM[code].name) || "your club";
  var signed = CG.signedExtensionOf(uid), dl = CG.capYearOpensAt(), open = CG.extensionWindowOpen();
  var live = (CG._myOffers||[]).filter(function(o){ return !o.immediate && o.from_team_id===c.team_id; })[0];
  var h = '<div class="card" style="margin-bottom:18px"><div class="card-h"><h3>Your contract</h3>';
  if (signed){
    var sc = ((CG.lg && CG.lg._idToCode) || {})[signed.team_id] || code;
    h += '<span class="chip chip-win">Signed through Season '+esc(String(signed.end_season))+'</span></div><div class="card-b">'+
      '<p class="small" style="color:var(--steel)">'+CG.fmtMoney(c.salary)+' this season with <b>'+esc(club)+'</b>, then <b>'+CG.fmtMoney(signed.salary)+'</b> a season from Season '+esc(String(signed.start_season))+' through Season '+esc(String(signed.end_season))+' with <b>'+esc((sc&&CG.TEAM[sc]&&CG.TEAM[sc].name)||club)+'</b>. The new deal comes into force with Season '+esc(String(signed.start_season))+'’s cap year, when your current one turns over (Rule 2.5).</p></div></div>';
    return h;
  }
  if ((c.end_season||1) > sn){
    h += '<span class="chip">Through Season '+esc(String(c.end_season))+'</span></div><div class="card-b">'+
      '<p class="small" style="color:var(--steel)">'+CG.fmtMoney(c.salary)+' a season with <b>'+esc(club)+'</b> through Season '+esc(String(c.end_season))+'. Re-signing talks open once the deal is in its final season — any time from that season’s free agency onward (Rule 2.2).</p></div></div>';
    return h;
  }
  /* final season */
  h += '<span class="chip chip-warn">Final season</span></div><div class="card-b">'+
    '<p class="small" style="color:var(--steel)">Your '+CG.fmtMoney(c.salary)+' deal with <b>'+esc(club)+'</b> ends after this season. ';
  if (!open){
    h += 'From <b>'+(dl?esc(CG.fmtFull(dl)):'the opening of this season’s free agency')+'</b> you and '+esc(club)+' can talk about a new one at any point in the season — either side can open the conversation. No other club may approach you, or you them, until free agency opens after the season (Rule 2.2).</p>';
  } else if (live){
    var theirs = CG.offerAwaitsClub(live);
    h += (theirs ? 'Your ask of <b>'+CG.fmtMoney(live.salary)+' × '+live.years+'</b> is with '+esc(club)+' — they can accept, revise, or walk away. <a href="#" id="withdrawAsk" data-offer="'+esc(live.id)+'" style="font-weight:700;border-bottom:2px solid var(--chrome)">Withdraw my ask</a>'
                 : esc(club)+' has offered you <b>'+CG.fmtMoney(live.salary)+' × '+live.years+'</b> from Season '+esc(String(live.start_season))+' — answer it in the offer card above.')+'</p>';
  } else {
    h += 'Re-signing talks are open for the whole season. '+esc(club)+' can offer you a new deal from Team HQ, or you can open with your own number. A new deal starts with next season’s cap year and is checked against the space '+esc(club)+' will have then. No other club may approach you until free agency opens after the season (Rule 2.2).</p>'+
      '<button class="btn btn-chrome btn-sm" id="askResign" style="margin-top:12px">'+CG.ic("plus",14)+'Ask to re-sign</button>';
  }
  return h + '</div></div>';
};
CG.wireExtensionCard = function(){
  var w = document.getElementById("withdrawAsk");
  if (w) w.addEventListener("click", function(e){
    e.preventDefault(); var id = this.getAttribute("data-offer");
    CG.confirm("Withdraw your ask?","The club will be told you took it back. You can send a new one any time in the window.","Withdraw", function(){
      CG.sb.rpc("respond_offer",{ p_offer:id, p_action:"deny", p_salary:null, p_years:null }).then(function(r){
        if (r.error){ CG.toast(r.error.message,"err"); return; }
        CG.toast("Ask withdrawn","ok"); CG.loadMyOffers().then(function(){ if (CG.router) CG.router(); });
      });
    });
  });
  var b = document.getElementById("askResign"); if (!b) return;
  b.addEventListener("click", function(){
    var uid = CG.auth.user.id, rights = !CG.contractOf(uid) && CG.rightsHeldContractOf(uid), c = CG.contractOf(uid) || rights, code = CG.contractClubCode(uid) || (c && ((CG.lg && CG.lg._idToCode) || {})[c.team_id]) || null;
    var club = (code && CG.TEAM[code] && CG.TEAM[code].name) || "your club", sn = (CG.SEASON && CG.SEASON.number) || 1;
    /* a rights-held re-sign is for the season now under way; an in-season extension is for the next */
    ...
    document.getElementById("rsGo").addEventListener("click", function(){
      var v=parseFloat(document.getElementById("rsSal").value), bad=CG.salaryProblem(Math.round(v*1e6));
      if (bad){ CG.toast(bad,"err"); return; }
      var y=parseInt(document.getElementById("rsYrs").value,10)||1, note=(document.getElementById("rsNote").value||"").trim()||null, btn=this; btn.disabled=true;
      CG.sb.rpc("request_extension",{ p_salary:Math.round(v*1e6), p_years:y, p_note:note }).then(function(r){
        btn.disabled=false;
        if (r.error){ CG.toast("Couldn’t send: "+r.error.message,"err"); return; }
        if (CG.closeOverlay) CG.closeOverlay();
        CG.toast("Sent — "+club+" has your ask","ok");
        CG.loadMyOffers().then(function(){ if (CG.router) CG.router(); });
      });
    });
  });
  ...
  document.querySelectorAll("[data-season-rollover]").forEach(function(b){ b.addEventListener("click", function(){
    ...
    CG.confirm("Run the rollover into "+name+"?",
      ...
      CG.sb.rpc("start_next_season",{ p_new_season:id }).then(function(r){
        ...
        var d=r.data||{};
        CG.toast("Rollover done — "+(d.expired||0)+" expired · "+(d.extended||0)+" extension"+((d.extended||0)===1?"":"s")+" in force · "+(d.activated||0)+" seated · "+(d.holds||0)+" awaiting sign-up","ok");
        CG.reloadLeague();
      });
    });
  }); });
  ...
  h+='<div class="card"><div class="card-h"><h3>The board</h3><span class="chip">'+pool.length+'</span></div>'+
    (pool.length?'<div class="tblwrap"><table class="tbl keepcols"><caption class="sr">Signable free agents</caption><thead><tr>'+
      ...
      pool.map(function(r){
        ...
        var held = !!(rh && rhCode && rhCode !== t.code);
  ...
  document.querySelectorAll("[data-fa-sign]").forEach(function(b){ b.addEventListener("click", function(){
    ...
    document.getElementById("faSignGo").addEventListener("click", function(){
      ...
      CG.sb.rpc("offer_free_agent",{ p_registration:regId, p_salary:sal, p_years:yrs, p_note:note }).then(function(r){
        ...
        if (r.error){ CG.toast("Couldn’t send: "+r.error.message,"err"); return; }
        if (CG.closeOverlay) CG.closeOverlay();
        CG.toast("Offer sent to "+name+" — "+CG.fmtMoney(sal)+" for "+yrs+" season"+(yrs>1?"s":"")+". He decides.","ok");
      });
      });
    });
  }); });
...
CG.cancelTrade = function(id){ CG.mgmtQueue("trade_cancel",{ p_trade:id }, "withdraw the trade offer to "+CG.tradePartnerName(id)).then(function(q){ if (q) return; CG.sb.from("trades").update({ status:"cancelled", updated_at:new Date().toISOString() }).eq("id",id).select("id").then(function(r){ if(r.error) CG.toast("Couldn’t withdraw: "+r.error.message,"err"); else if(!r.data||!r.data.length) CG.toast("Couldn’t withdraw — the offer may have changed. Refresh and retry.","err"); else { CG.toast("Offer withdrawn","ok"); CG.refreshTrades(); } }); }); };
/* the other club's name for a trade id — for the Owner's approval summaries */
CG.tradePartnerName = function(id){
  var tr=(CG.lg._myTrades||[]).find(function(x){ return x.id===id; }); if(!tr) return "another club";
  var mine=(CG.lg._codeToId||{})[CG.myClub()], other=tr.from_team_id===mine?tr.to_team_id:tr.from_team_id;
  var code=(CG.lg._idToCode||{})[other]; return (code && CG.TEAM[code] && CG.TEAM[code].name) || "another club";
};
