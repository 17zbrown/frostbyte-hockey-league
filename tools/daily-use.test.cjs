...
console.log("— member writes can't report a success the server refused");
{
  ...
  /* v2.96 added preferred_server to the same write; pin the guard, not the column list */
  A("the settings twin too", /update\(\{ ea_id:ea\|\|null, platform:plat\|\|null,[^}]*\}\)[\s\S]{0,80}\.select\("id"\)/.test(live));
}
