...
/* v3.40 wrapped this in the position-lock setting: with the lock off flex() is true for everybody,
   and the pre-season management exception is what survives for a season that turns the lock on. */
A("a pre-season game lets management fill any slot (camp players always could)",
  /function flex\(p\)\{ return !locksOn \|\| p\.squad==="tc" \|\| \(preGame && !!p\.mgmt\); \}/.test(hub));
A("...and with the lock lifted, everybody fills any slot", /var locksOn = !CG\.posLocksOn \|\| CG\.posLocksOn\(\);/.test(hub));
A("...validate, the bench hint, the slot targets and auto-fill all use the one test", (hub.match(/flex\(p\)/g) || []).length >= 4);
A("...auto-fill ranks a rostered player at his own position first, then a borrowed manager, then camp (Rule 5.2 P2 kept)", /var rank = function\(p\)\{ var a = avState\(p\); return \(a==="no"\?8:a==="nr"\?4:0\) \+ \(p\.squad==="tc"\?2:0\) \+ \(CG\.posGroup\(p\.pos\)!==CG\.posGroup\(pos\)\?1:0\); \};/.test(hub));   /* v2.76: availability ranks ahead of the squad order */
...
console.log("\n— the line creator");
A("while the club's next game is a pre-season game, management sits anywhere on a line", /var preAhead = !!\(CG\.preseasonOnlyAhead && CG\.preseasonOnlyAhead\(club\)\);/.test(hub) && /if \(\(!CG\.posLocksOn \|\| CG\.posLocksOn\(\)\) && p\.squad!=="tc" && !\(preAhead && p\.mgmt\)\)\{/.test(hub));
...
console.log("\n— the rulebook says it (rendered JSON)");
{
  ...
  A("5.2: whatever is in force is applied when a lineup is filed, not only when built", /apply whatever is in force when a lineup is filed, not only when it is built/.test(sec("5.2")));
  /* BASIC format has no pre-season, so management gets no dressing exception — camp is the only one */
  A("[basic] 2.1: while the lock is on, exactly one exception to the groups — training camp", /the sole exception is a training-camp player, who fills any position for as long as he is carried in camp/.test(sec("2.1")));
