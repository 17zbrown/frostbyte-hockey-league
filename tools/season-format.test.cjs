...
A("2.1: season-settings roster shape, management inside, depth in camp", /determine, for each season, the size of the active roster and its composition by position group/.test(sec("2.1")) && /shall publish these figures to the clubs before that season's draft/.test(sec("2.1")) && /counted within the published composition in their own position groups/.test(sec("2.1")) && /is carried in the club's training camp as depth/.test(sec("2.1")) && /Camp is not counted against the published composition/.test(sec("2.1")));
