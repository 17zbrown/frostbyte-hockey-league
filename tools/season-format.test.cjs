...
A("basic is the standard (v2.51 layout): 15 = two lines + three flex (caps 9 F / 7 D / 5 G), camp unlimited, everyone 6 a week (camp 3), 4 a series, 16-GP floor (v2.67 default), $50M, 6 weeks, deadline week 4, 15 snake rounds, 1-season deals, no extensions/rights/picks/pre-season/FA, top 3, best of 7",
  ...
  CG.FORMAT_RULES.basic.camp_max === 999 && CG.FORMAT_RULES.basic.cap_skater === 6 && CG.FORMAT_RULES.basic.cap_camp === 3 && CG.FORMAT_RULES.basic.series_cap === 4 && CG.FORMAT_RULES.basic.playoff_min_gp === 16 &&
...
/* v3.26 re-pinned: 2.1 used to LEAD with the quota form ("its composition by position group — the
   number of forwards, defensemen and goaltenders") and offer lines-plus-flex as an option. That is
   the full format's shape. Under basic the composition IS lines plus flex and the group maxima are
...
A("2.2: no market, waived players signable from the draft's conclusion, no rights classes", /no free-agency period: no window in the calendar during which contracts are negotiated, and no open market in which a club bids/.test(sec("2.2")) && /From the conclusion of the draft until the movement deadline/.test(sec("2.2")) && /The basic format recognizes no classes of player rights/.test(sec("2.2")));
...
A("8.3: best-of-seven 2-2-3, three a series for everyone, byes", /best-of-seven series played within a single game-week in a 2-2-3 format/.test(sec("8.3")) && /no player — skater, goaltender or training-camp player — may be dressed in more than four \(4\) games of a single series/.test(sec("8.3")) && /the playoff eligibility floor\) and shall publish it with the season settings before that season's draft/.test(sec("8.3")) && !/eighteen|\b18\b/.test(sec("8.3")) && /Rules 2\.1, 2\.8 and 8\.3/.test(sec("10.1")) && /Three figures are season settings/.test(sec("0.1")) && /first seed plays no series in the opening week/.test(sec("8.3")));
...
A("...and the binding 2.1 is the basic one", basicBook.chapters[2].sections.find((s) => s.id === "2.1").paragraphs.join(" ").includes("determine, for each season, the size and composition of the active roster"));
A("full: the roles swap — 2.1 binds with seventeen, Appendix A holds the basic text", fullBook.format === "full" && fullBook.chapters[2].sections.find((s) => s.id === "2.1").paragraphs.join(" ").includes("seventeen (17) players") && appx(fullBook).sections.find((s) => s.of === "2.1").paragraphs.join(" ").includes("determine, for each season, the size and composition of the active roster") && /basic format/.test(appx(fullBook).title));
