...
A("loans render in their own block below the contracted players, with the rule explained", /var rows = contracted\.map\(rowFor\)\.join\(""\) \+/.test(hub) && /Pre-season loans — '\+loans\.length\+'/.test(hub) && /they return to the draft pool when the final pre-season game ends \(Rule 0\.4\)/.test(hub));
